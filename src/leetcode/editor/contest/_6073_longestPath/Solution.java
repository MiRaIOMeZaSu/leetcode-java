package leetcode.editor.contest._6073_longestPath;

import java.util.*;

class TreeNode {
    char val;
    int index;

    TreeNode(char val, int index) {
        this.val = val;
        this.index = index;
    }
}

class Solution {
    private int ans = 1;
    private Map<Integer, List<TreeNode>> childrenListMap;

    public int longestPath(int[] parent, String s) {
        childrenListMap = new HashMap<>(parent.length);
        // build tree
        Map<Integer, TreeNode> nodeMap = new HashMap<>();
        TreeNode root = new TreeNode(s.charAt(0), 0);
        nodeMap.put(0, root);
        for (int i = 1; i < parent.length; i++) {
            TreeNode node = new TreeNode(s.charAt(i), i);
            List<TreeNode> child = childrenListMap.get(parent[i]);
            if (childrenListMap.get(parent[i]) == null) {
                child = new ArrayList<>();
                childrenListMap.put(parent[i], child);
            }
            child.add(node);
            nodeMap.put(i, node);
        }
        solve(root);
        return ans;
    }

    private int solve(TreeNode node) {
        int nextMax = 0;
        PriorityQueue<Integer> priorityQueue = new PriorityQueue<>(2, Integer::compareTo);
        List<TreeNode> children = childrenListMap.get(node.index);
        for (int i = 0; children != null && i < children.size(); i++) {
            TreeNode child = children.get(i);
            int childMax = solve(child);
            if (child.val != node.val) {
                nextMax = Math.max(nextMax, childMax);
                priorityQueue.add(childMax);
                while (priorityQueue.size() > 2) {
                    priorityQueue.poll();
                }
            }
        }
        int temp = 0;
        while (!priorityQueue.isEmpty()) {
            temp += priorityQueue.poll();
        }
        ans = Math.max(ans, temp + 1);
        return nextMax + 1;
    }

    public static void main(String[] args) {
//        Solution solution = new Solution();
//        solution.longestPath(new int[]{-1, 0, 0, 1, 1, 2}, "abacbe");
//        Solution solution = new Solution();
//        solution.longestPath(new int[]{-1, 0, 1}, "aab");
        Solution solution = new Solution();
        solution.longestPath(new int[]{-1, 137, 65, 60, 73, 138, 81, 17, 45, 163, 145, 99, 29, 162, 19, 20, 132, 132, 13, 60, 21, 18, 155, 65, 13, 163, 125, 102, 96, 60, 50, 101, 100, 86, 162, 42, 162, 94, 21, 56, 45, 56, 13, 23, 101, 76, 57, 89, 4, 161, 16, 139, 29, 60, 44, 127, 19, 68, 71, 55, 13, 36, 148, 129, 75, 41, 107, 91, 52, 42, 93, 85, 125, 89, 132, 13, 141, 21, 152, 21, 79, 160, 130, 103, 46, 65, 71, 33, 129, 0, 19, 148, 65, 125, 41, 38, 104, 115, 130, 164, 138, 108, 65, 31, 13, 60, 29, 116, 26, 58, 118, 10, 138, 14, 28, 91, 60, 47, 2, 149, 99, 28, 154, 71, 96, 60, 106, 79, 129, 83, 42, 102, 34, 41, 55, 31, 154, 26, 34, 127, 42, 133, 113, 125, 113, 13, 54, 132, 13, 56, 13, 42, 102, 135, 130, 75, 25, 80, 159, 39, 29, 41, 89, 85, 19},
                "ajunvefrdrpgxltugqqrwisyfwwtldxjgaxsbbkhvuqeoigqssefoyngykgtthpzvsxgxrqedntvsjcpdnupvqtroxmbpsdwoswxfarnixkvcimzgvrevxnxtkkovwxcjmtgqrrsqyshxbfxptuvqrytctujnzzydhpal");
    }
}